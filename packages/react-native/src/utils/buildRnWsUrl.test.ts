...
describe('buildRnWsUrl', () => {
  ...
  it('converts http:// to ws://', () => {
    const url = buildRnWsUrl('http://api.relaya.chat', 'my-space');
  ...
  it('appends station slug as a query parameter', () => {
    const url = buildRnWsUrl('https://api.relaya.chat', 'my-space');
    expect(url).toBe('wss://api.relaya.chat/ws?station=my-space');
  });
  ...
  it('omits token param when token is undefined (anonymous connection)', () => {
    const url = buildRnWsUrl('https://api.relaya.chat', 'my-space', undefined);
    expect(url).toBe('wss://api.relaya.chat/ws?station=my-space');
    expect(url).not.toContain('token=');
  });

  it('includes token param before station when token is provided', () => {
    const url = buildRnWsUrl('https://api.relaya.chat', 'my-space', 'mytoken123');
    expect(url).toBe('wss://api.relaya.chat/ws?token=mytoken123&station=my-space');
  });
  ...
  it('URI-encodes a JWT token containing +, /, and = characters', () => {
    ...
    const url = buildRnWsUrl('https://api.relaya.chat', 'my-space', token);
