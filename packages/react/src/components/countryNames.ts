...
/**
 * ISO 3166-1 alpha-2 country code → full country name mapping.
 * Used by GeoRestrictionsAdmin for the searchable country picker.
 */
...
/** Returns "Country Name (CC)" display string, or just CC if name is unknown. */
export function countryLabel(cc: string): string {
  const name = COUNTRY_NAMES[cc.toUpperCase()];
  return name ? `${name} (${cc.toUpperCase()})` : cc.toUpperCase();
}

/** Returns "Country Name · CC" chip label. */
export function countryChipLabel(cc: string): string {
  const name = COUNTRY_NAMES[cc.toUpperCase()];
  return name ? `${name} \u00b7 ${cc.toUpperCase()}` : cc.toUpperCase();
}

/** All entries sorted by country name for dropdown display. */
export const SORTED_COUNTRIES: { code: string; name: string }[] =
