...
**Server:** Requires Relaya server v1.6.0 or later (for terms acceptance support; v1.5.0 works but omits `termsAccepted`/`termsUrl`/`termsVersion` from auth responses - the SDK defaults `termsAccepted: true` for backward compatibility). The hosted SaaS (`api.relaya.chat`) always runs the current version.
